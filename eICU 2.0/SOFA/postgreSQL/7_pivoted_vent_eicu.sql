-- Ventilation information for eICU Collaborative Research Database 2.0
-- 20240930
...
drop table if exists eicu_crd.pivoted_vent_part1_eicu;
create table eicu_crd.pivoted_vent_part1_eicu as

with ventaliation_info as (
	select cplgeneralid
	, patientunitstayid
	, case
	when activeupondischarge = 'True' then true			
	else false
	end as activeupondischarge	
	, cplitemoffset
	, cplgroup
	, cplitemvalue
	, case 
...
drop table if exists eicu_crd.pivoted_vent_part2_eicu;
create table eicu_crd.pivoted_vent_part2_eicu as
...
with ventilation_20 as (
	...
	from eicu_crd.pivoted_vent_part1_eicu
)
...
drop table if exists eicu_crd.pivoted_vent_part34_eicu;
create table eicu_crd.pivoted_vent_part34_eicu as
...
with ventilation_30 as (
	...
	from eicu_crd.pivoted_vent_part2_eicu
	where vent_flag = 0
)
...
, ventilation_40 as (
	...
	from (select distinct * from ventilation_3) as foo
)
...
drop table if exists eicu_crd.pivoted_vent_part56_eicu;
create table eicu_crd.pivoted_vent_part56_eicu as
...
with ventilation_50 as (
	...
	, ROW_NUMBER()
	OVER (partition by patientunitstayid order by cplitemoffset desc) as rn
	from eicu_crd.pivoted_vent_part34_eicu
)
...
, ventilation_61 as (
	...
	left join eicu_crd.icustay_detail icud 
...
drop table if exists eicu_crd.pivoted_vent_eicu;
create table eicu_crd.pivoted_vent_eicu as
...
with ventilation_70 as (
	...
	from eicu_crd.pivoted_vent_part56_eicu
)
