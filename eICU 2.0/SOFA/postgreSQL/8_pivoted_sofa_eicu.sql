-- SOFA information for eICU Collaborative Research Database 2.0
-- 20240930
...
drop table if exists eicu_crd.pivoted_sofa_eicu;
create table eicu_crd.pivoted_sofa_eicu as
...
with cohort_info_0 as (
  ...
  from eicu_crd.icustay_detail icud
  where age != ''
)
...
(
  select patientunitstayid
  , GENERATE_ARRAY(-24, CAST(ceil(los_icu_hours) AS integer)) as hr
  from cohort_info
)
...
(
  ...
  from eicu_crd.pivoted_blood_pressure_eicu pv
...
(
  ...
  from pafi_0 pf
  left join eicu_crd.pivoted_vent_eicu pv
...
, uo as (
  ...
  from  co
  left join eicu_crd.pivoted_uo uo
...
(
  ...
  left join eicu_crd.norepinephrine_info_eicu ni 
  ...
  left join eicu_crd.epinephrine_info_eicu ei 
  ...
  left join eicu_crd.dobutamine_info_eicu di 
  ...
  left join eicu_crd.dopamine_info_eicu dop 
